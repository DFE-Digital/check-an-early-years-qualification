...
public class CachingHandlerTests(TestContext testContext)
{
    ...
    public async Task Request_ReadsFromCache_UsingUrlAsKey()
    {
        ...
        response.Content.ReadAsByteArrayAsync(TestContext.CancellationToken).Result.Should()
    ...
    public async Task Request_NotFoundInCache_CallsBaseSend_AndWritesToCache()
    {
        ...
        var cached = await distributedCache.GetAsync(key, TestContext.CancellationToken);
