...
public class HelpControllerTests
{
    ...
    [TestMethod]
    public async Task Post_QualificationDetails_ValidModelState_RedirectsToProvideDetails()
    {
        // Arrange
        ...
        _mockHelpService
            .Setup(x => x.MapHelpQualificationDetailsPageContentToViewModel(submittedViewModel, content))
        ...
        _mockHelpService.Verify(x => x.SetHelpQualificationDetailsInCookie(submittedViewModel), Times.Once);
        _mockHelpService.Verify(x => x.AddQualificationDetailsValidationErrors(submittedViewModel, content, It.IsAny<ModelStateDictionary>()), Times.Once);
    }

    [TestMethod]
    public async Task Post_QualificationDetails_ContentReturnsNull_RedirectsToError()
    {
        // Arrange
        var submittedViewModel = new QualificationDetailsPageViewModel();
    ...
    [TestMethod]
    public async Task Post_QualificationDetails_InvalidModelState_ReturnsViewWithModel()
    {
        // Arrange
        var content = new HelpQualificationDetailsPage
                      {
        ...
        _mockHelpService
            .Setup(x => x.MapHelpQualificationDetailsPageContentToViewModel(vm, content))
            .Returns(vm);
        ...
        var resultType = result as ViewResult;
        resultType.Should().NotBeNull();
        resultType.ViewName.Should().Be("QualificationDetails");

        var model = resultType.Model as QualificationDetailsPageViewModel;
        model.Should().NotBeNull();

        _mockHelpService.Verify(x => x.AddQualificationDetailsValidationErrors(vm, content, It.IsAny<ModelStateDictionary>()), Times.Once);
        _mockHelpService.Verify(x => x.SetHelpQualificationDetailsInCookie(It.IsAny<QualificationDetailsPageViewModel>()), Times.Never);
    }

    [TestMethod]
    public async Task Post_QualificationDetails_ModelHasErrors_ReturnsViewWithModel()
    {
        // Arrange
        var content = new HelpQualificationDetailsPage
                      {
        ...
        var vm = new QualificationDetailsPageViewModel
                 {
                     QualificationName = "qualification name",
                     AwardingOrganisation = "awarding org",
                     Errors = new List<ErrorSummaryLink> { new ErrorSummaryLink { ErrorBannerLinkText = "Test error", ElementLinkId = "#test" } }
                 };
        ...
        // Act
        var result = await GetSut().QualificationDetails(vm);
