...
public class CookieServiceTests
{
  ...
  private Mock<IHttpContextAccessor> SetContextWithPreferenceCookie(DfeCookie cookie)
  {
    ...
    requestCookiesMock.Setup(cookiesCollection => cookiesCollection["cookies_preferences_set"]).Returns(serializedCookie);
    responseCookiesMock.Setup(x => x.Delete(It.IsAny<string>())).Verifiable();

    var httpContextMock = new Mock<IHttpContextAccessor>();
    httpContextMock.Setup(contextAccessor => contextAccessor.HttpContext!.Request.Cookies).Returns(requestCookiesMock.Object);
    httpContextMock.Setup(contextAccessor => contextAccessor.HttpContext!.Response.Cookies).Returns(responseCookiesMock.Object);
    return httpContextMock;
  }

  private void CheckSerializedCookieWasSet(Mock<IHttpContextAccessor> mockContext, string serializedCookieToCheck)
  {
    mockContext.Verify(
        x => x.HttpContext!.Response.Cookies.Append(
          ...
          It.Is<CookieOptions>(
            cookieOptions =>
              ...
              cookieOptions.Expires > new DateTimeOffset(DateTime.Now.AddDays(364)) &&
              cookieOptions.Expires < new DateTimeOffset(DateTime.Now.AddYears(1)))), Times.Once);
  }
}