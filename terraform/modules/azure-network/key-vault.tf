...
resource "azurerm_key_vault_certificate" "kv_test_rotation_cert" {
  # Test Certificate only deployed to the Test subscription
  count = var.environment == "staging" ? 1 : 0

  name         = var.kv_test_certificate_label
  key_vault_id = azurerm_key_vault.kv.id
  ...
  certificate_policy {
    ...
    x509_certificate_properties {
      extended_key_usage = ["1.3.6.1.5.5.7.3.1", "1.3.6.1.5.5.7.3.2"]
      key_usage          = ["digitalSignature", "keyEncipherment"]
      subject            = var.kv_test_certificate_subject
